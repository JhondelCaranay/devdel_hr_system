...
exports.employeeRoleAccessData = exports.financeRoleAccessData = exports.hrRoleAccessData = exports.ceoRoleAccessData = exports.hrAccessData = exports.homeAccessData = void 0;
const env_1 = require("@/config/env");
const role_access_1 = require("./../schema/role_access");
...
async function seeder() {
    const db = (0, node_postgres_1.drizzle)(env_1.ENV.DATABASE_URL);
    ...
    // CREATE ACCESS
    await createAccesssByModule(db, exports.homeAccessData, modules.find((m) => m.name === "Home").id);
    await createAccesssByModule(db, exports.hrAccessData, modules.find((m) => m.name === "HR").id);
    ...
    await createRoleAccessByRole(db, roles.find((r) => r.name === "ceo").id, exports.ceoRoleAccessData);
    await createRoleAccessByRole(db, roles.find((r) => r.name === "hr").id, exports.hrRoleAccessData);
    await createRoleAccessByRole(db, roles.find((r) => r.name === "finance").id, exports.financeRoleAccessData);
    await createRoleAccessByRole(db, roles.find((r) => r.name === "employee").id, exports.employeeRoleAccessData);
...
seeder();
const createUserByRole = async (db, data, role_id, username) => {
    const insertedUsers = await db
        .insert(schema.users)
        .values({ ...data, role_id })
        .returning();
    // create credential
    const hashedPassword = await (0, hashPassword_1.hashPassword)("password1234");
    await (0, auth_services_1.createCredential)({
...
const createAccesssByModule = async (db, data, module_id) => {
...
const createRoleAccessByRole = async (db, role_id, accessCodes) => {
    const accesses = await db.select().from(schema.access).where((0, drizzle_orm_1.inArray)(schema.access.code, accessCodes));
    const roleAccessData = accesses.map((access) => ({ role_id, access_id: access.id }));
    const insertedRoleAccess = await db.insert(role_access_1.roleAccess).values(roleAccessData).returning();
...
const adminRoleAccessData = [
    // home
    ...exports.homeAccessData.map((access) => access.code),
    // hr
    ...exports.hrAccessData.map((access) => access.code),
    // finance
...
exports.employeeRoleAccessData = [
    // home
    "home:view_overview_page",
    "notice_board:view_notice_board_page",
    // profile
